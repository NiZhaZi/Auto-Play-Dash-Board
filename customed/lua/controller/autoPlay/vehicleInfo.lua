...
local function updateGFX(dt)
    ...
    local fuelReMiAge
    if averageFuelConsumption > 0.1 then
        fuelReMiAge = remainingFuel / averageFuelConsumption * 100 -- km
    else
        fuelReMiAge = remainingFuel / 50 * 100 -- km
    end

    local elecReMiAge
    if averageElectricityConsumption > 0.1 then
        elecReMiAge = remainingElectricity / averageElectricityConsumption * 100 -- km
    else
        elecReMiAge = remainingElectricity / 50 * 100 -- km
    end

    remainingMileAge = fuelReMiAge + elecReMiAge -- km
    range.past = mileage
    range.rest = remainingMileAge
